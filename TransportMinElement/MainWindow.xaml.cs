    ...
    public partial class MainWindow : Window
    {
        ...
        private void Solve_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                int[,] plan = MinimalElementMethod(costs, supply, demand, out int totalCost, out string log);
        ...
        private int[,] MinimalElementMethod(int[,] costs, int[] supplyInput, int[] demandInput, out int totalCost, out string log)
        {
            ...
            bool[] rowFinished = new bool[m];
            bool[] colFinished = new bool[n];

            totalCost = 0;
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("Ход решения методом минимального элемента:");
            sb.AppendLine();
            ...
            while (true)
            {
                ...
                for (int i = 0; i < m; i++)
                {
                    if (rowFinished[i]) continue;

                    for (int j = 0; j < n; j++)
                    {
                        if (colFinished[j]) continue;
                ...
                int cellCost = x * costs[minI, minJ];
                totalCost += cellCost;

                sb.AppendLine($"Выбрана минимальная стоимость: C[{minI + 1},{minJ + 1}] = {costs[minI, minJ]}");
                sb.AppendLine($"Назначаем X[{minI + 1},{minJ + 1}] = min({supply[minI]}, {demand[minJ]}) = {x}");
                sb.AppendLine($"Стоимость в клетке: {x} * {costs[minI, minJ]} = {cellCost}");

                supply[minI] -= x;
                demand[minJ] -= x;

                sb.AppendLine($"Остаток запаса A{minI + 1}: {supply[minI]}");
                sb.AppendLine($"Остаток потребности B{minJ + 1}: {demand[minJ]}");

                if (supply[minI] == 0)
                {
                    rowFinished[minI] = true;
                    sb.AppendLine($"Строка A{minI + 1} закрыта.");
                }

                if (demand[minJ] == 0)
                {
                    colFinished[minJ] = true;
                    sb.AppendLine($"Столбец B{minJ + 1} закрыт.");
                }

                sb.AppendLine();

                bool allRowsDone = rowFinished.All(xr => xr);
                bool allColsDone = colFinished.All(xc => xc);

                if (allRowsDone || allColsDone)
                    break;
            }
        ...
        private void ExportCsv_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                SaveFileDialog saveFileDialog = new SaveFileDialog
                {
                    ...
                    FileName = "transport_min_element_result.csv"
                };
